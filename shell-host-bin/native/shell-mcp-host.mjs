...
async function handleCallTool(id, params) {
  ...
  if (name === 'shell_status') {
    const shellInfo = createShellStatusData();
    return jsonRpcResult(id, {
      content: [{ type: 'text', text: `Shell host ready on ${shellInfo.platform} ${shellInfo.arch}. Current shell: ${shellInfo.shell}. Working directory: ${shellInfo.cwd}. Use ${shellInfo.shellFamily} syntax, not Windows PowerShell commands unless shell is powershell.exe.` }],
      structuredContent: {
        ok: true,
        data: shellInfo,
      },
...
function execCommand(command, { cwd, env, timeoutMs }) {
  return new Promise((resolve, reject) => {
    const translated = translateCrossPlatformShellCommand(command);
    const commandToRun = translated?.command || command;
    const shellHintError = translated?.error || detectMismatchedShellSyntax(commandToRun);
    if (shellHintError) {
      resolve({
        ...
        exitCode: 127,
        signal: null,
        stdout: translated?.note ? translated.note : '',
        stderr: shellHintError,
        truncated: false,
        timedOut: false,
      });
      return;
    ...
    const { shellBin, shellArgs } = createShellInvocation(commandToRun);
...
function translateCrossPlatformShellCommand(command) {
  if (platform() === 'win32') return null;
  const text = String(command || '').trim();
  const expandedFullNamePattern = /^Get-ChildItem\s+-(?:LiteralPath|Path)\s+(.+?)\s+-Recurse(?:\s+-ErrorAction\s+\S+)?(?:\s+-Include\s+(\S+))?\s*\|\s*Select-Object\s+-ExpandProperty\s+FullName\s*$/i;
  const match = text.match(expandedFullNamePattern);
  if (match) {
    const rawPath = normalizePowerShellPathToken(match[1]);
    const includePattern = normalizePowerShellIncludePattern(match[2]) || '*';
    const translatedPath = translatePowerShellSearchPath(rawPath);
    return {
      command: 'find ' + quotePosixPath(translatedPath) + ' -type f -iname ' + quotePosixPath(includePattern) + ' -print',
      note: 'Translated Windows PowerShell file search to POSIX find for this host. Original path: ' + rawPath + '. Translated path: ' + translatedPath + '.',
    };
  }
  return null;
}
...
function normalizePowerShellPathToken(value) {
...
function normalizePowerShellIncludePattern(value) {
  if (typeof value !== 'string' || !value.trim()) return null;
  return normalizePowerShellPathToken(value.trim());
}

function translatePowerShellSearchPath(rawPath) {
  const value = String(rawPath || '').trim();
  if (!value) return homedir();
  if (/^\$env:USERPROFILE$/i.test(value)) return homedir();
  if (/^\$env:HOME$/i.test(value)) return homedir();
  if (/^[A-Za-z]:\\Program\*$/i.test(value)) return '/Applications';
  if (/^[A-Za-z]:\\$/i.test(value)) return '/';
  if (/^[A-Za-z]:\\/i.test(value)) return '/';
...
function createShellStatusData() {
  const shell = DEFAULT_SHELL;
  const shellName = basename(shell || 'shell').toLowerCase();
  const shellFamily = platform() === 'win32'
    ? (shellName.includes('powershell') || shellName === 'pwsh' ? 'powershell' : 'cmd')
...
function detectMismatchedShellSyntax(command) {
  if (platform() === 'win32') return null;
  const text = String(command || '');
  const looksLikePowerShell = /\bGet-ChildItem\b|\bSelect-Object\b|\bGet-Command\b|\$env:/i.test(text);
  if (!looksLikePowerShell) return null;
  return [
    'This host is running on ' + platform() + ' with shell ' + DEFAULT_SHELL + '.',
    'The command appears to use Windows PowerShell syntax (for example Get-ChildItem or Select-Object), which is not valid in this shell.',
    'Call shell_status first and use the reported shell syntax.',
    'For macOS/Linux, use commands such as: find . -iname "*jenkins*" or ls -la.',
  ].join(' ');
}
